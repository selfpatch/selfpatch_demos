#!/bin/bash
# Trigger the SOVD install flow: deploy obstacle_classifier_v2 from scratch.

set -eu

GATEWAY_URL="${OTA_GATEWAY_URL:-http://localhost:${OTA_GATEWAY_PORT:-8080}}"
API="${GATEWAY_URL}/api/v1"
ID="obstacle_classifier_v2_1_0_0"
...
if docker ps --format '{{.Names}}' | grep -q '^ota_demo_gateway$'; then
    echo ""
    echo "Live processes:"
    docker exec ota_demo_gateway pgrep -af 'obstacle_classifier' \
